...
const newsletterSubscribersCollectionName = process.env.DB_NEWSLETTER_SUBSCRIBERS_COLLECTION;
...
export async function newsletterSignup(newsletterSubscriberData) {
    const response = await fetch(`${FIREBASE_DOMAIN}/${newsletterSubscribersCollectionName}.json`, {
        method: 'POST',
...
export async function getCommentsPerEvent(eventId) {
    ...
    const data = await response.json();
    console.log('getCommentsPerEvent -> data = ', data);
    console.log('getCommentsPerEvent -> query = ', query);
